...
class TokenCacheService(
    ...
    fun isTokenFamilyRevoked(userId: Long, token: String): Boolean {
        return try {
            ...
            redisTemplate.opsForSet().isMember(familyKey, token) ?: false
        } catch (e: Exception) {
    ...
    fun isLoginLocked(email: String): Boolean {
        return try {
            val lockoutKey = getLoginLockoutKey(email)
            redisTemplate.hasKey(lockoutKey) ?: false
        } catch (e: Exception) {
    ...
    fun revokeAllUserTokens(userId: Long) {
        try {
            val familyKey = getTokenFamilyKey(userId)
            val tokenIds = redisTemplate.opsForSet().members(familyKey) ?: emptySet()
            
            tokenIds.forEach { tokenId ->
                val revokedKey = getRevokedTokenKey(tokenId.toString())
                redisTemplate.opsForValue().set(revokedKey, "revoked", Duration.ofDays(30))
            }
            
            // Limpar família de tokens
            redisTemplate.delete(familyKey)
            
            log.info("Todos os tokens do usuário $userId foram revogados")
        } catch (e: Exception) {
            log.error("Erro ao revogar todos os tokens do usuário $userId", e)
        }
    }

    /**
     * Verifica se uma chave está sob rate limiting
     */
    fun isRateLimited(key: String, durationMinutes: Int): Boolean {
        return try {
            val rateLimitKey = getRateLimitKey(key)
            redisTemplate.hasKey(rateLimitKey)
        } catch (e: Exception) {
            log.error("Erro ao verificar rate limit para $key", e)
            false
        }
    }

    /**
     * Aplica rate limiting para uma chave
     */
    fun applyRateLimit(key: String, durationMinutes: Int) {
        try {
            val rateLimitKey = getRateLimitKey(key)
            redisTemplate.opsForValue().set(
                rateLimitKey, 
            ...
            log.debug("Rate limit aplicado para $key por $durationMinutes minutos")
        } catch (e: Exception) {
            log.error("Erro ao aplicar rate limit para $key", e)
        }
    }
