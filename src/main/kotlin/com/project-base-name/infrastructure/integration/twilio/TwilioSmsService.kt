...
import com.projectbasename.infrastructure.integration.twilio.dto.SendSmsDto
import com.projectbasename.infrastructure.integration.twilio.dto.SmsResponseDto
...
class TwilioSmsService(
    ...
    @Value("\${twilio.webhook.enabled:false}") private val webhookEnabled: Boolean
) : TwilioSmsProvider {
    ...
    override fun sendSms(phone: String, message: String): String {
        ...
        try {
            ...
            // Só adiciona callback se não for localhost
            if (!isLocalEnvironment()) {
                log.debug("========================================== Enviando ---------------------------------------")

                messageCreator.setStatusCallback(getCallbackUrl())
            }

            val twilioMessage = messageCreator.create()
    ...
    private fun isLocalEnvironment(): Boolean {
        return apiServerUrl.contains("localhost") || apiServerUrl.contains("127.0.0.1")
    }
